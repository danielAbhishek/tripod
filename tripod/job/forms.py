...
from job.models import Task, Work, Workflow, Job
from job.workflow.workflow import WorkFlowBase
...
class WorkForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if self.operation == 'updating':
            self.obj = Work.objects.get(pk=self.instance.pk)
        add_basic_html_tags("Work", self.fields, True)
    ...
    def save(self, *args, **kwargs):
        if self.operation == 'creating':
            ...
            self.instance.id = self.obj.id
            self.instance.changed_by = self.user
        workObj = super(WorkForm, self).save(*args, **kwargs)
        return workObj
